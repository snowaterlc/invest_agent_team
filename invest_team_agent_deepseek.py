...
def get_a_share_data(ts_code: Optional[str] = None, limit_data: bool = True) -> dict:
    ...
    try:
        ...
        try:
            ...
            if any([balance_has_data, income_has_data, indicator_has_data, prime_has_data, valuation_has_data]):
                ...
                if balance_has_data:
                    ...
                    if fina_data_dict["ttl_liab"] and fina_data_dict["total_assets"]:
                        fina_data_dict["debt_ratio"] = (fina_data_dict["ttl_liab"] / fina_data_dict["total_assets"]) * 100
                    else:
                ...
                if valuation_has_data:
                    ...
                    fina_data_dict["circulating_market_value"] = (row.get("neg_mkt_cap") / 100000000) if row.get("neg_mkt_cap") else None
...
def compliance_check(stock_list: list, analysis_report: str) -> dict:
    ...
    return {
        ...
        "suggestions": ["修正违规内容", "补充风险提示"] if total_pass < len(compliance_rules) else ["合规"]
    }
...
def web_plagiarism_check(report_content: str) -> dict:
    """网上查重工具"""
    try:
        sentences = [s.strip() for s in report_content.split("\n") if len(s.strip()) > 20][:3]
        if not sentences: return {"plagiarism_score": 0, "is_plagiarized": False}

        matches = 0
        for s in sentences:
            try:
                resp = requests.get(f"https://www.baidu.com/s?wd={quote(s)}", timeout=5)
                if s in resp.text: matches += 1
            except: pass
...
agents = [
    Agent(
        ...
        verbose=True, llm=llm, tools=[get_a_share_data], max_iter=10
    ),
    Agent(
        role="股性活跃度分析师",
    ...
    Agent(
        ...
        verbose=True, llm=llm, tools=[compliance_check], max_iter=5
    ),
    Agent(
        role="内容原创审核员",
        goal="确保报告内容的原创性，防止抄袭",
        backstory="严格审核每一份文稿，维护分析工作的独立性。",
        verbose=True, llm=llm, tools=[web_plagiarism_check], max_iter=5
    ),
    Agent(
        role="小盘股投资顾问",
        goal=f"汇总分析，生成最终报告",
        backstory="整合各方观点，给出简洁明了的行动指南。",
        verbose=True, llm=llm, allow_delegation=True, max_iter=10
    ),
]
