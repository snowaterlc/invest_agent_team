...
# ====================== 数据库配置（SQLite版）======================
def create_sqlite_engine(db_path: str = None):
    """
    创建SQLite数据库引擎对象
    :param db_path: 数据库文件路径，默认 ./data/investment.db
    """
    if db_path is None:
        db_path = os.getenv("SQLITE_DB_PATH", "./data/investment.db")
    ...
    try:
        ...
        logger.error(f"SQLite数据库连接失败: {str(e)}")
        return None
...
def save_report_to_db(report_content, selected_stocks):
    ...
    try:
        ...
        # 保存投资报告
        report_date_obj = datetime.strptime(NEXT_TRADING_DAY, "%Y-%m-%d").date()
        report = InvestmentReport(
            report_date=report_date_obj, report_content=report_content
        )
        session.add(report)
        session.commit()
        ...
        # 保存选中的股票信息
        buy_date_obj = datetime.strptime(NEXT_TRADING_DAY, "%Y-%m-%d").date()
        ...
        session.commit()
        session.close()
        logger.info(f"投资报告和选中股票已保存到SQLite数据库，报告ID: {report_id}")
    except Exception as e:
        logger.error(f"保存到SQLite数据库失败: {str(e)}")
        import traceback
